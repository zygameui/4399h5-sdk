...
class Main:
    _hx_class_name = "Main"
    __slots__ = ()
    _hx_statics = ["exts", "main", "check"]

    @staticmethod
    def main():
        haxe_Log.trace("开始检查",_hx_AnonObject({'fileName': "Main.hx", 'lineNumber': 7, 'className': "Main", 'methodName': "main"}))
        Main.check(python_internal_ArrayImpl._get(Sys.args(), 0))

    @staticmethod
    def check(dirfile):
        if (not sys_FileSystem.exists(dirfile)):
            haxe_Log.trace("文件夹不存在：",_hx_AnonObject({'fileName': "Main.hx", 'lineNumber': 13, 'className': "Main", 'methodName': "check", 'customParams': [dirfile]}))
            return
        array = sys_FileSystem.readDirectory(dirfile)
        _g = 0
        while (_g < len(array)):
            file = (array[_g] if _g >= 0 and _g < len(array) else None)
            _g = (_g + 1)
            if sys_FileSystem.isDirectory(((("null" if dirfile is None else dirfile) + "/") + ("null" if file is None else file))):
                Main.check(((("null" if dirfile is None else dirfile) + "/") + ("null" if file is None else file)))
            else:
                startIndex1 = None
                pos = None
                ...
                tmp = None
                if (python_internal_ArrayImpl.indexOf(Main.exts,ext,None) != -1):
                    startIndex = None
                    tmp = (((file.find(" ") if ((startIndex is None)) else HxString.indexOfImpl(file," ",startIndex))) != -1)
                else:
                    tmp = True
                if tmp:
                    haxe_Log.trace(("需要过滤：" + ("null" if file is None else file)),_hx_AnonObject({'fileName': "Main.hx", 'lineNumber': 23, 'className': "Main", 'methodName': "check"}))
...
Main.exts = "htm,html,js,css,jpg,jpeg,gif,png,mp3,ogg,m4a,xml,csv,dat,atf,json,plist,wav,thm,fnt,exportjson,ttf,tmx,exml,datagz,jsgz,memgz,unityweb,scml,scon,ani,atlas,lani,lh,lmat,lm,sk,ls,lav,wasm,pck,fui,ltc,prefab,part,ltcb,lamat,txt,dbbin".split(",")
